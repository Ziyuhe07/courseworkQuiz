...
//This code is adopted from https://leafletjs.com/examples/custom-icons/
//Figures also downloaded from the website
...
// This function adjusted add earthquake layer in week 2 practice 
// Give global variable of question form data layer
	var formDatalayer;
//getPOI -view data as GeoJSON
	...
	// transform the received data to JSON format (object) and add it to the map
	function loadFormDatalayer(formData) {
	// convert the text to JSON
		var formDatajson = JSON.parse(formData);
	// add the JSON layer onto the map - this used the default marker to make more distinct comparison of 
	// the most closed one 
	        	    ...
	        	    for (geometry in feature[component]){
                             ...
                             var distance = calculateDistance(position.coords.latitude, position.coords.longitude, lat,lng, 'K');
                                 document.getElementById('showDistance').innerHTML = "Distance: " + distance;
                                       if (distance<0.15){  //set distance of 0.15km, assuming the lower accuracy of mobile positioning
	                                           L.marker([lat, lng],{icon: greenIcon}).addTo(mymap).bindPopup("<b>I am approaching</b>").openPopup();
											   //add customised icon, the green leaf
...
// Use for loops to obtain questions and answers value from database
...
// Set the answers value, so it can update when position changes 
...
var client;
// Process Data through the httpServer.js to database
function processData(postString) {
	client = new XMLHttpRequest();
	client.open('POST','http://developer.cege.ucl.ac.uk:30271/uploadQuizData',true);//the url should altered 
...
// change the DIV to examine the response
